...
// LaserProcessing::LaserProcessing(sensor_msgs::Image image, sensor_msgs::Image depth):
//     image_(image), depth_(depth), squareDetected_(false), rotationAngle_(0), centreX_(0), centreY_(0)
// {
//     turn_ = 0;
//     left_ = false;
//     right_ = false;
...
// bool LaserProcessing::testMessages(double& rotationAngle, int& turn, bool& left, bool& right) {
//     ROS_INFO_STREAM("image test");
//     ROS_INFO_STREAM(image_.width);
//     ROS_INFO_STREAM(image_.height);
//     ROS_INFO_STREAM("depth test");
//     ROS_INFO_STREAM(depth_.width);
//     ROS_INFO_STREAM(depth_.height);
//     convertToGreyscale();
//     if(squareDetected_) {
//         // double rotationAngle_ = findRotationAngle(topLeft_, topRight_, bottomLeft_, bottomRight_);
//         findOrientation();
//         std::cout << "Rotation angle: " << rotationAngle_ << " degrees" << std::endl;
...
// void LaserProcessing::findOrientation() {
//     left_ = false;
//     right_ = false;
//     double rotationAngle_ = findRotationAngle(topLeft_, topRight_, bottomLeft_, bottomRight_);
//     if(rotationAngle_<5) turn_ = 0;
//     if(centreX_ - image_.width/2 < -15) left_ = true;
//     if(centreX_ - image_.width/2 > 15) right_ = true;
//     if(rotationAngle_>5) {
//         if(topLeft_.y > topRight_.y) turn_ = 1;
//         else turn_ = 2;
//     }
// }
...
//         int num_depth_values = width * height;
//         const float* depth_value = reinterpret_cast<const float*>(depth_.data[0]);
        
//         for (int i = 0; i < num_depth_values; ++i) {
//             float depth_value = depth_.data[i];
//             if (!std::isnan(depth_value) && depth_value != 0.0f) {
//                 valid_depths_.push_back(depth_value);
...
//                 }
//                 // if (depth_.data[j * depth_.width + i] != 0.0 && !(std::isnan(depth_.data[j * depth_.width + i])) && !(std::isinf(depth_.data[j * depth_.width + i]))) {
//                 //     std::cout << "depth: " << depth_.data[j * depth_.width + i] << std::endl;
//                 // }
//                 // if (std::is_floating_point<decltype(depth_.data[j * depth_.width + i])>::value) {
//                 //     std::cout << "depth: " << depth_.data[j * depth_.width + i] << std::endl;
//                 // }
//                 // if(depth_.data[j * depth_.width + i] != 0) {
//                 //     std::cout << "depth: " << depth_.data[j * depth_.width + i] << std::endl;
//                 //     // depthSum += depth_.data[j * depth_.width + i];
//                 //     // depthCount++;
//                 //     // std::cout << "true";
//                 // }
//             }
//         }
//     }
//     std::cout << "Average depth: " << depthSum/depthCount << std::endl;
//     if(corner_x_coords.size()>3 && corner_x_coords.size()<50) {
...
// double LaserProcessing::distance( LaserProcessing::CoordPoint p1,  LaserProcessing::CoordPoint p2) {
//     return std::sqrt(std::pow(p2.x - p1.x, 2) + std::pow(p2.y - p1.y, 2));
// }

// double LaserProcessing::findRotationAngle( LaserProcessing::CoordPoint topLeft,  LaserProcessing::CoordPoint topRight,
//                                  LaserProcessing::CoordPoint bottomLeft,  LaserProcessing::CoordPoint bottomRight) {
//     double apparentWidth = distance(topLeft, topRight);
//     double apparentHeight = distance(topLeft, bottomLeft);

//     // Assuming that the original square, when not rotated, would have its width
//     // equal to the height in the perspective view:
//     double originalWidth = apparentHeight;

//     double cosTheta = apparentWidth / originalWidth;

//     // Ensure value is between -1 and 1 due to potential floating-point inaccuracies
//     if (cosTheta > 1.0) cosTheta = 1.0;
//     if (cosTheta < -1.0) cosTheta = -1.0;

//     return std::acos(cosTheta) * (180.0 / M_PI);  // Convert radian to degree
// }
...
//     //topLeft3D
//         topLeft3D_.x = (topLeft_.x-cx)*((d*fy)/(bottomLeft_.y-topLeft_.y))*(1/fx);
//         topLeft3D_.y = (topLeft_.y-cy)*((d*fy)/(topLeft_.y-bottomLeft_.y))*(1/fy);
//         topLeft3D_.z = ((d*fy)/(bottomLeft_.y-topLeft_.y));
//     //topRight3D
//         topRight3D_.x = (topRight_.x-cx)*((d*fy)/(bottomRight_.y-topRight_.y))*(1/fx);
//         topRight3D_.y = (topRight_.y-cy)*((d*fy)/(topRight_.y-bottomRight_.y))*(1/fy);
//         topRight3D_.z = ((d*fy)/(bottomRight_.y-topRight_.y));
//     //bottomLeft3D
//         bottomLeft3D_.x = (bottomLeft_.x-cx)*((d*fy)/(bottomLeft_.y-topLeft_.y))*(1/fx);
//         bottomLeft3D_.y = (bottomLeft_.y-cy)*((d*fy)/(topLeft_.y-bottomLeft_.y))*(1/fy);
//         bottomLeft3D_.z = ((d*fy)/(bottomLeft_.y-topLeft_.y));
//     //bottomRight3D
//         bottomRight3D_.x = (bottomRight_.x-cx)*((d*fy)/(bottomRight_.y-topRight_.y))*(1/fx);
//         bottomRight3D_.y = (bottomRight_.y-cy)*((d*fy)/(topRight_.y-bottomRight_.y))*(1/fy);
//         bottomRight3D_.z = ((d*fy)/(bottomRight_.y-topRight_.y));
...
//     p1.x = topLeft3D_.x - topRight3D_.x;
//     p1.y = topLeft3D_.y - topRight3D_.y;
//     p1.z = topLeft3D_.z - topRight3D_.z;
//     p2.x = bottomRight3D_.x - topRight3D_.x;
//     p2.y = bottomRight3D_.y - topRight3D_.y;
//     p2.z = bottomRight3D_.z - topRight3D_.z;
//     //calculate normal1
...
//     p3.x = bottomLeft3D_.x - topRight3D_.x;
//     p3.y = bottomLeft3D_.y - topRight3D_.y;
//     p3.z = bottomLeft3D_.z - topRight3D_.z;
//     //calculate normal2

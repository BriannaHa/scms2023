...
void LaserProcessing::testMessages() {
    ...
    // // OpenCV
    // src = imread( "grayscale_image.pgm", IMREAD_COLOR );
    int channels = src.channels();
    // std::cout << "Number of channels: " << channels << std::endl;
    int dims = src.dims;
    std::cout << "Number of dimensions: " << dims << std::endl;
    if(dims>0) {
        if(channels==3 || channels ==4) cvtColor( src, src_gray, COLOR_BGR2GRAY );
        else src_gray = src;
...
void LaserProcessing::convertToGreyscale() {
    ...
    if (imageEncoding == "mono8") {
        ...
        if (greyscale_image_initialised_){
            ...
            ROS_INFO("CONVERTED");
            cv::Mat image(height, width, CV_8UC1, greyscale_image_.data());
            src = image;
        }
        return;
    }
...
void LaserProcessing::cornerHarrisTest(int, void*)
{
